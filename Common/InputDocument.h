...
// for DOCX files
	...
	mz_zip_archive m_docxZipArchive;
	std::vector<uint8_t> m_docxXmlBuffer;
	size_t m_docxXmlPos;

// for DOC files
	int OpenDoc(wchar_t* filename);
	void CloseDoc();
	std::wstring m_docTextBuffer;
	size_t m_docTextPos;
	bool m_useIFilter;
...
// for PDF files
	...
	ptrdiff_t FindStringInBufferPdf(char* haystack, const char* needle, size_t haystackLength);
