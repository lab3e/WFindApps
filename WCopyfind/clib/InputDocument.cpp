...
// check if the pdftotext.exe conversion program is present in the same directory as WCopyfind.exe
	...
	memset(&m_docxZipArchive, 0, sizeof(m_docxZipArchive));
	m_docxXmlPos = 0;
	m_useIFilter = false;
	m_docTextPos = 0;
...
int CInputDocument::GetWord(wchar_t *word,int &DelimiterType)
{
	...
	{
		if (m_useIFilter) // IFilter extracted text into m_docTextBuffer
		{
			while (true)
			{
				if (m_docTextPos >= m_docTextBuffer.size())
				{
					word[wordLength] = 0;
					DelimiterType = DEL_TYPE_EOF;
					return -1;
				}
				wchar_t wc = m_docTextBuffer[m_docTextPos++];
				if (wc == L'\n' || wc == L'\r')
				{
					DelimiterType = DEL_TYPE_NEWLINE;
					m_gotDelimiter = true;
				...
				{
					DelimiterType = max(DelimiterType, DEL_TYPE_WHITE);
					m_gotDelimiter = true;
				}
				else if (iswcntrl(wc)) continue;
				else if (m_gotDelimiter)
				{
					if (m_gotWord)
					{
						word[wordLength] = 0;
						m_docTextPos--; // un-consume wc so first char of next word is not lost
						return -1;
					}
					else
					{
						DelimiterType = DEL_TYPE_NONE;
						m_gotDelimiter = false;
						m_docTextPos--; // un-consume wc so it is re-read in next iteration
					}
				}
				else if (wordLength < WORDMAXIMUMLENGTH)
				{
...
int CInputDocument::OpenDoc(wchar_t* filename)
{
	...
	if (SUCCEEDED(LoadIFilter(filename, nullptr, (void**)&pFilter)))
	{
		ULONG flags = 0;
		pFilter->Init(IFILTER_INIT_APPLY_INDEX_ATTRIBUTES, 0, nullptr, &flags);
		STAT_CHUNK chunk;
		wchar_t buf[4096];
		while (pFilter->GetChunk(&chunk) == S_OK)
		{
	...
	// Fallback: byte-scan (existing approach)
...
{
	// Read the entire zip into memory via wide-char path (avoids 256-char wcstombs_s limit)
	FILE* fp = nullptr;
	...
	fread(zipData.data(), 1, fileSize, fp);
	fclose(fp);

	memset(&m_docxZipArchive, 0, sizeof(m_docxZipArchive));
	if (!mz_zip_reader_init_mem(&m_docxZipArchive, zipData.data(), zipData.size(), 0))
		return ERR_CANNOT_OPEN_INPUT_FILE;

	size_t xmlSize = 0;
	void* xmlData = mz_zip_reader_extract_file_to_heap(&m_docxZipArchive, "word/document.xml", &xmlSize, 0);
	mz_zip_reader_end(&m_docxZipArchive);

	if (!xmlData) return ERR_BAD_DOCX_FILE;

	m_docxXmlBuffer.assign((uint8_t*)xmlData, (uint8_t*)xmlData + xmlSize);
	mz_free(xmlData);
	m_docxXmlPos = 0;
...
int CInputDocument::OpenPdf(wchar_t* filename)
{
	...
	{
		// Build command line: "path\pdftotext.exe" -enc UTF-8 "path\file.pdf" -
		std::wstring cmdLine = L"\"" + std::wstring(m_pdftotextFile) + L"\" -enc UTF-8 \"" + std::wstring(filename) + L"\" -";
		...
		if (!CreatePipe(&hReadPipe, &hWritePipe, &sa, 0))
			return ERR_CANNOT_OPEN_INPUT_FILE;
		SetHandleInformation(hReadPipe, HANDLE_FLAG_INHERIT, 0); // read end is not inherited
		...
		if (fd == -1) { CloseHandle(hReadPipe); return ERR_CANNOT_OPEN_INPUT_FILE; }
		m_filep = _fdopen(fd, "r");
		if (!m_filep) return ERR_CANNOT_OPEN_INPUT_FILE;
	...
	else // don't have pdftotext program avialable, so use my simple-minded pdf decoding functions
	{
		...
		int fileBufferLength = (int)m_fileLength;
		...
		int itemsRead = (int)fread(m_fileBuffer, fileBufferLength, 1 , m_filep);
		...
		int bytesRemaining = (int)m_fileLength; // get decrementable copy of fileLength

		m_ByteBufferLengthPdf = (int)(m_fileLength*10); // make text buffer 10 times as large as file (should be enough space)
		...
		while (true)
		{
			...
			ptrdiff_t streamStart = FindStringInBufferPdf (fileBufferPointer, "stream", bytesRemaining);
			ptrdiff_t streamEnd   = FindStringInBufferPdf(fileBufferPointer, "endstream", bytesRemaining);
			if (streamStart >= 0 && streamEnd > streamStart)
			{
				...
				//Now use zlib-compatible inflate (via miniz):
				z_stream zstrm = {};

				zstrm.avail_in = (unsigned int)(streamEnd - streamStart + 1);
				zstrm.avail_out = (unsigned int)inflateBufferSize;
				...
				bytesRemaining -= (int)(streamEnd + 7);
			}
			else
			{
				break;
...
{
	if (m_docxXmlPos < m_docxXmlBuffer.size())
		return m_docxXmlBuffer[m_docxXmlPos++];
	return -1;
}
...
//Find a string (needle) in a buffer (haystack). Returns -1 if not found.
ptrdiff_t CInputDocument::FindStringInBufferPdf(char* haystack, const char* needle, size_t haystackLength)
{
	char* haystack0 = haystack;
	size_t needleLength = strlen(needle);
	if (needleLength > haystackLength) return -1;

	for(ptrdiff_t position=0; position <= (ptrdiff_t)(haystackLength-needleLength); position++)
	{
